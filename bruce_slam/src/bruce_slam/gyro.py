...
set_global_logger(self.get_logger())
	...
	def init_node(self, ns:str="~")->None:
		...
		x = self.get_parameter(ns + "offset/x").value  # CHECK: declare_parameter needed first
		...
		self.latitude = np.radians(self.get_parameter(ns + "latitude").value  # CHECK: declare_parameter needed first)
		self.earth_rate = -15.04107 * np.sin(self.latitude) / 3600.0
		self.sensor_rate = self.get_parameter(ns + "sensor_rate").value  # CHECK: declare_parameter needed first
