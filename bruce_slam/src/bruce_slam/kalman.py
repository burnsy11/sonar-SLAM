...
set_global_logger(self.get_logger())
	...
	def init_node(self, ns="~")->None:
		...
		self.state_vector = self.get_parameter(ns + "state_vector").value  # CHECK: declare_parameter needed first
		self.cov_matrix = self.get_parameter(ns + "cov_matrix").value  # CHECK: declare_parameter needed first
		self.R_dvl = self.get_parameter(ns + "R_dvl").value  # CHECK: declare_parameter needed first
		self.dt_dvl = self.get_parameter(ns + "dt_dvl").value  # CHECK: declare_parameter needed first
		self.H_dvl = np.array(self.get_parameter(ns + "H_dvl").value  # CHECK: declare_parameter needed first)
		self.R_imu = self.get_parameter(ns + "R_imu").value  # CHECK: declare_parameter needed first
		self.dt_imu = self.get_parameter(ns + "dt_imu").value  # CHECK: declare_parameter needed first
		self.H_imu = np.array(self.get_parameter(ns + "H_imu").value  # CHECK: declare_parameter needed first)
		self.H_gyro = np.array(self.get_parameter(ns + "H_gyro").value  # CHECK: declare_parameter needed first)
		self.R_gyro = self.get_parameter(ns + "R_gyro").value  # CHECK: declare_parameter needed first
		self.dt_gyro = self.get_parameter(ns + "dt_gyro").value  # CHECK: declare_parameter needed first
		self.H_depth = np.array(self.get_parameter(ns + "H_depth").value  # CHECK: declare_parameter needed first)
		self.R_depth = self.get_parameter(ns + "R_depth").value  # CHECK: declare_parameter needed first
		self.dt_depth = self.get_parameter(ns + "dt_depth").value  # CHECK: declare_parameter needed first
		self.Q = self.get_parameter(ns + "Q").value  # CHECK: declare_parameter needed first # Process Noise Uncertainty
		self.A_imu = self.get_parameter(ns + "A_imu").value  # CHECK: declare_parameter needed first # State Transition Matrix
		x = self.get_parameter(ns + "offset/x").value  # CHECK: declare_parameter needed first # gyroscope offset matrix
		y = self.get_parameter(ns + "offset/y").value  # CHECK: declare_parameter needed first
		z = self.get_parameter(ns + "offset/z").value  # CHECK: declare_parameter needed first
		...
		self.imu_offset = np.radians(self.get_parameter(ns + "imu_offset").value  # CHECK: declare_parameter needed first)

		# check which version of the imu we are using
		if self.get_parameter(ns + "imu_version").value  # CHECK: declare_parameter needed first == 1:
			self.imu_sub = rospy.Subscriber(IMU_TOPIC, Imu,callback=self.imu_callback,queue_size=250)
		elif self.get_parameter(ns + "imu_version").value  # CHECK: declare_parameter needed first == 2:
